...
editor = os.getenv("EDITOR") or "editor" -- sudo update-alternatives --config editor
...
local default_layout_portrait = 2
...
mymainmenu = menu.build({after = myawesomemenu})
...
awful.screen.connect_for_each_screen(function(s)
    ...
    if s.geometry.height > s.geometry.width then
        -- portrait 
        layout_idx = default_layout_portrait
    end

    -- Each screen has its own tag table.
    awful.tag({ "❶", "❷", "❸", "❹", "❺", "❻", "❼", "❽", "❾" }, s, awful.layout.layouts[layout_idx])
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            layout = wibox.layout.fixed.horizontal,
            separators.group_left(wibox.widget.systray()),
...
globalkeys = awful.util.table.join(
    ...
    -- Custom
    awful.key({ modkey,           }, "q", function () awful.spawn(browser_command) end,
              {description = "launch web browser", group = "launcher"}),
    awful.key({ modkey,           }, "c", function () awful.spawn(file_browser_command) end,
    ...
    awful.key({ modkey, "Shift"   }, "Left",
       function (c)
		  ...
		  local current_tags = awful.screen.focused().tags
          if curidx == 1 then
              awful.client.movetotag(current_tags[#current_tags])
          else
              awful.client.movetotag(current_tags[curidx - 1])
          end
          awful.tag.viewidx(-1)
      end,
		{description = "Shift client left", group = "tag"}),
    awful.key({ modkey, "Shift"   }, "Right",
      function (c)
          local curidx = awful.tag.getidx()
		  local current_tags = awful.screen.focused().tags
          if curidx == #current_tags then
              awful.client.movetotag(current_tags[1])
          else
              awful.client.movetotag(current_tags[curidx + 1])
          end
          awful.tag.viewidx(1)
      end,
		{description = "shift client right", group = "tag"})
)
